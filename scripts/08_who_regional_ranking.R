...
tryCatch({
  ...
  if (!is.null(paths$who_sheet_name) && paths$who_sheet_name %in% sheets) {
    who_raw <- readxl::read_excel(paths$emdat_xlsx, sheet = paths$who_sheet_name)
  } else if (!is.null(paths$who_sheet_index) && paths$who_sheet_index <= length(sheets)) {
    who_raw <- readxl::read_excel(paths$emdat_xlsx, sheet = paths$who_sheet_index)
  } else {
    stop(sprintf(
      "WHO sheet not found. Available sheets: %s\nCheck config: who_sheet_name / who_sheet_index.",
  ...
  if (!all(needed %in% names(who))) {
    stop(sprintf(
      "WHO sheet must contain columns: Region, Cause, Death (any case).\nFound: %s",
  ...
  region_mapping <- list(
    "Africa" = c(
      "Algeria","Angola","Benin","Burundi","Central African Republic","Chad",
    ...
    "Europe" = c(
      ...
      "Netherlands (Kingdom of the)","Poland","Romania","Russian Federation",
  ...
  message("Check that your WHO sheet name/index is correct in config/config.yml")
})
